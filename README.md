...
- **选股复盘（真实胜率+超额）** — 四个选股池（形态/智能/波段/竞价）每次扫描自动留痕，按真实行情统计 T+1/T+3/T+5 胜率、平均收益与**相对全市场中位的超额收益**（区分策略能力与大盘涨跌）；数据同步缺口自动检测、自动补齐并从统计中排除；数据说话，不承诺胜率。
- **历史回放验证** — 用本地日线把形态/智能两池规则 point-in-time 回放 12 个月（每 5 个交易日采样、每期 top-N），输出月度超额胜率与累计超额曲线，选股规则有效还是无效直接看数据。
- **入选理由卡** — 选股列表每只票可点开「理由」抽屉：因子分解、入选理由、交易计划、该信号的历史 T+5 超额表现（实盘留痕 + 历史回放双口径）。
- **模拟组合** — 选股列表一键「+组合」按真实价格整手虚拟买入（含交易成本），收盘自动结算净值 vs 全市场中位；持仓触发卖出信号（跌破 MA20 / 止损 -8% / 超时未盈利）自动提示，解决「只管买不管卖」。
