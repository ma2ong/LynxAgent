...
- 新模块 `quantcore/quant/report_daily.py`：
  - 盘前版（工作日 9:26，竞价结束后开盘前——9:00 时竞价数据尚不存在）：
    复用竞价候选、催化剂 → LLM 写「今日看点」。
  - 收盘版（工作日 15:35）：复用环境标签、涨停主线、资金面板数据 → LLM 写
...
- 后端 `GET /api/lite/macro-bar`：上证/深成/创业板指数涨跌（腾讯 s_ 简版行情）、
  涨跌家数、两市成交额（复用实时快照）；60s 缓存。
  不含北向净流入——沪深港通实时资金流 2024 年起已停止盘中披露，无可靠数据源。
