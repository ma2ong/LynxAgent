...
⚠ 2026-09-09：本文件产出的 results/auction_rule.json（A_current +0.438pp / t=2.57）
已判定不可用，两处口径问题各自都会把数字抬上去：

1. 候选用 `amt < MIN_AMOUNT` 筛，而 amt 是**当日全天成交额** —— 09:25 决策时不存在，
   等于先知道哪些票今天会放量再回头挑它们；
2. 组合取均值、基准 `med` 取中位。README「口径」一节已经写过这条：个股收益右偏，
   实测全市场均值比中位高 +0.134pp/日（盘中）、+0.196pp/日（T+1）。

改成昨日成交额 + 两边同为均值后，同一条规则从 +0.438pp 变成 −0.33pp。
新口径见 `auction_tradeable_ab.py`（另补了本文件缺的「卖点」维度：A 股 T+1，
当日买入最早次日才能卖，本文件的 T+1 收盘口径是对的，但它没有报出
「当日收盘」那一列其实不可交易这件事）。

本文件保留不删：它是 2026-07-28 那次决策的现场记录。要重跑请用新文件。
