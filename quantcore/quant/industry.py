...
def industry_stage_scores(
    ...
    **2026-08-05 改：主升段由罚 30 分改为奖 12 分**（按 Allen 的决定：不惩罚正在
    ...
    for ind, rows in members.items():
        ...
        score = 50.0
        # 2026-08-28 重配系数。rule_audit 六年全样本、次日开盘买入口径，把这三项
        # 各自当规则审了一遍（对照增量 / CI 下沿 / 去右尾，T+5 与 T+10）：
        #   板块20日涨幅  +0.14 / +0.06 / +0.42  ← 三项里唯一每关都稳的
        #   板块5日涨幅   +0.11 / +0.02 / +0.29
        #   量能扩张      +0.03 / −0.04 / +0.18  ← CI 下沿跨 0，**过不了闸**
        # 也就是说原来系数 40 的主导项本身没有 alpha，这个因子一直是靠系数 1.8 的
        # mom5 在扛。现在按实测强度重配：mom20 主导(±40)、mom5 不动、量能扩张退成
        # 辅助(±10)。不直接删掉量能扩张，是因为它还参与下面的主升段/潜伏判定。
        score += _clip(mom20, -20.0, 20.0) * 2.0     # 板块中期动量：主导项
        score += _clip(mom5, -8.0, 8.0) * 1.8        # 近期在涨加分
        score += _clip(vol_exp, -0.5, 1.0) * 10      # 放量加分、缩量减分
