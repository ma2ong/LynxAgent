...
# 这里用**绝对金额**而不是横截面分位：分位会随大盘放量缩量自动漂移，看到「第 54 分位」
# 判断不出这个板块到底有没有人交易；一个能直接读懂的金额更适合拿来做决定。
#
# 2026-09-02 定 100 亿，2026-09-04 降到 20 亿。100 亿是个假的「流动性下限」：全市场
# 121 个板块的 5 日成交额中位数才 65 亿，100 亿等于要求整个板块日均 20 亿，一刀砍掉
# 六成板块，跟买不买得进无关。实测当日只有 2 个板块能同时过它和 5% 涨幅，主榜几乎全被
# 折叠。个股层面已经有 LEADER_MIN_AMOUNT（日均 1 亿）的硬约束，板块层面再卡 100 亿
# 是重复设卡。20 亿/5 日 ≈ 全板块日均 4 亿，保留的是「没人交易的板块别上榜」这个本意。
SECTOR_MIN_AMOUNT = 20e8
